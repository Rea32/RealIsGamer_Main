...
if (isset($_GET["page"])) {
    ...
    $result=$conn->query('SELECT id_juego FROM juego');
    
    while($row=$result->fetch_array()){
        $juego_activo = new \Juego\Clase\juego($row[0]);
        $juego_activo->setPunt_usuarios($juego_activo->getPunt_usuarios());
        
    }
    
    $result = $conn->query(
        'SELECT id_juego FROM juego ORDER BY punt_usuarios DESC LIMIT '.$start.', '.NUM_ITEMS_BY_PAGE
    );

    
    if ($result->num_rows > 0) { 

        echo '<div class="d-lg-flex flex-column justify-content-center align-items-center" >';
        while ($row = $result->fetch_assoc()) {
            $juego = new Juego\Clase\juego($row['id_juego']);
            $info_juego = $juego->obtenerTodaInfo();
        ?>
            <div class="contJuego d-md-flex mt-3" id="<?php echo $ids_juegos[$i];?>">
                <div id="notaCont" class="notasConts d-md-flex justify-content-center align-items-center" style="min-width:80px" >
                ...
                <p class="p-3 d-none d-xl-flex"><?php echo $info_juego['descripcion']; ?></p>
            </div>
        <?php } 
    ...
    if ($total_pages > 1) {
        if ($page != 1) {
            echo '<li class="page-item"><a class="page-link" href="Ranking.php?page='.($page-1).'"><span aria-hidden="true">Pag Anterior</span></a></li>';
        }
 
        for ($i=1;$i<=$total_pages;$i++) {
            if ($page == $i) {

                echo '<li class="page-item active"><a class="page-link" href="Ranking.php?page=">'.$page.'</a></li>';
    
            } else {

                echo '<li class="page-item"><a class="page-link" href="Ranking.php?page='.$i.'">'.$i.'</a></li>';
        ...
        if ($page != $total_pages) {
                ...
                echo '<li class="page-item"><a class="page-link" href="Ranking.php?page='.($page+1).'"><span aria-hidden="true">Siguiente Página</span></a></li>';
